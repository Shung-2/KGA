...
void enemyManager::setMinion()
{
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			enemy* ufo;
			ufo = new minion;
			ufo->init("enemy", PointMake(80 + j * 80, 80 + i * 100), 100);
...
{
	_boss = new boss();
	_boss->init("boss", PointMake(WINSIZEX / 2, -50), 1000);
	_vMinion.push_back(_boss);
}

// ## 210528 ufo move, boss ##
void enemyManager::hitDamage(int _idx, float damage)
{
	_vMinion[_idx]->hitDamage(damage);
	if (_vMinion[_idx]->getHP() == 0)
	{
		removeMinion(_idx);
	}
}